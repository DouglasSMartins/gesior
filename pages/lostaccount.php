...
if($config['site']['send_emails']){
	if(!$logged)	{
		...
		if($_REQUEST['step'] == "sendconfirmation") {
			...
			if($email != $account->getEmail() || $account_name != $account->getName()) {
				...
				$emailcode = $SQL->prepare("INSERT INTO links (account_id, code, code_date) VALUES (:accid, :ncode , :cdate)");
               			$emailcode->execute(array('accid' => $idAccount, 'ncode'=> $newcode, 'cdate' => $codeDate));
	    ...
	    $getEmail = filter_var($getEmail, FILTER_SANITIZE_STRING);
            ...
            $getEmail->execute(array('email'=> $email));
		...
		$emailcode = filter_var($emailcode, FILTER_SANITIZE_STRING);
                ...
                $emailcode->execute(array('accid'=> $idAccount, 'code'=> $newcode, 'code_date'=> $codeDate));
		...
		if($_REQUEST['step'] == "sendpassword") {
            ...
            $key->execute(array('confirmation_key'=>$confirmationkey));
            $key = $key->fetchAll();
            ...
            $code_account->execute(array('accid'=>$accountID));
            ...
            if (empty($error)) {
                ...
                $delCode->execute(array('ckey'=> $confirmationkey));
